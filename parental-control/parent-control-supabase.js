...
(function () {
  ...
  window.parentControl = {
    ...
    // Activity Intelligence — farzand faoliyati (backend authorization: active link SHART).
    async getChildActivity(childEmail, days) {
      const b = bridge();
      if (!b || typeof b.getChildActivity !== 'function') return { ok: false, error: 'Ulanish yo\'q.' };
      return b.getChildActivity(String(childEmail || '').toLowerCase(), days || 14);
    },

    // Haftalik hisobotni yaratib, ota-onaga yuborish (email + notification).
    async sendWeeklyReport(childEmail) {
      const b = bridge();
      if (!b || typeof b.sendWeeklyReport !== 'function') return { ok: false, error: 'Ulanish yo\'q.' };
      return b.sendWeeklyReport(String(childEmail || '').toLowerCase());
    },
