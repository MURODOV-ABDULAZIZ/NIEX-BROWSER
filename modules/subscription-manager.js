...
const PLAN_DEFINITIONS = {
  free: {
    ...
    dailyLimits: {
      ...
      videoMinutes: 60,    // Video AI: 60 daqiqa/kun (premium sahifadagi va'daga mos)
      image:        Infinity,
      deepScan:     Infinity,
      ocr:          Infinity,
      pdf:          Infinity,
    ...
    localUnlimited: true,
    limits: { imageAnalyses: Infinity, videoMinutes: 60, textAnalysis: Infinity, urlAnalysis: Infinity },
    features: {
      ...
      // Voice Agent va Redirect Protection — PRO obunachilar uchun (foydalanuvchi qarori).
      voiceAgent: false,
      redirectProtection: false,
    },
  },
  pro: {
    id: 'pro',
...
class SubscriptionManager {
  constructor(options = {}) {
    ...
    // Joriy hisob DISKDAN tiklanadi — aks holda ilova har ochilganda currentAccount=null
    //   bo'lardi va Pro FAQAT premium sahifa ochilganda (self-heal) aniqlanardi. Buning oqibati:
    //   voice agent ilova ochilishi bilan ko'rinmasди — foydalanuvchi Pro tugmani bosishi kerak edi.
    //   Endi oxirgi hisob tiklanadi → _voiceAllowed() darhol Pro'ni topadi. (Xavfsiz: lastAccount
    //   faqat "qaysi hisob" hint'i; Pro haqiqiyligini state.accounts[email].planId belgilaydi.)
    this.currentAccount = this.state.lastAccount || null;
  }
  ...
  _loadState() {
    try {
      ...
      if (parsed.accounts && typeof parsed.accounts === 'object') {
        const st = { accounts: parsed.accounts };
        if (parsed.lastAccount) st.lastAccount = String(parsed.lastAccount).toLowerCase();
        return st;
      }
  ...
  //   Joriy hisobni DISKDA ham saqlaymiz (keyingi ochilishда tiklansin).
  setCurrentAccount(email) {
    const next = email ? String(email).toLowerCase() : null;
    this.currentAccount = next;
    if (this.state.lastAccount !== next) {
      if (next) this.state.lastAccount = next; else delete this.state.lastAccount;
      this._saveState();
    }
  ...
  getUpgradeOffer() {
    return {
      ...
      price: 22000, originalPrice: 55000, currency: 'UZS', planId: 'pro',
    };
  }
