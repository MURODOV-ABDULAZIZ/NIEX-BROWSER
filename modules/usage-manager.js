...
class UsageManager {
  ...
  recordUsage(options = {}) {
    ...
    // Yangi per-op tracker'ga ham qo'shamiz (mapping). MUHIM: LOCAL video (kadr-skaner)
    //   HAM videoMinutes'ga qo'shiladi — aks holda "0/60" turib qolardi (foydalanuvchi video
    //   ko'rdi, lekin daqiqa hisoblanmadi). cost sekundlarda → daqiqaga (kasr, aniq to'planadi).
    if (type === 'video' || type === 'video-local') {
      this.consume('videoMinutes', cost / 60, analysisId);
